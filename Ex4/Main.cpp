...
bool is_valid_new_room(int room_x, int room_y)
{
    ...
    // check bottom
    if (room_y < row_count-1 && active_rooms.count(std::to_string(room_x) + ',' + std::to_string(room_y+1)))
        neighbor_count++;

    // check right
    if (room_x < col_count-1 && active_rooms.count(std::to_string(room_x+1) + ',' + std::to_string(room_y)))
        neighbor_count++;
...
bool try_add_room(int room_x, int room_y, int previous_index)
{
    ...
    if (room_x < 0 || room_x > col_count-1 || room_y < 0 || room_y > row_count-1)
    {
...
void generate_dungeon()
{
    ...
    room_count = GetRandomValue(10, 20);
    rooms.reserve(room_count);

    std::cout << "(" << room_count << " rooms)\n\n"
                << "STARTING ROOM\n"; 

    add_new_room(GetRandomValue(0, col_count-1), GetRandomValue(0, row_count-1), SKYBLUE, 0, nullptr);
    ...
    while (rooms.size() < room_count)
    {
        ...
        try_add_room(room.x+1, room.y, current_index);
...
void draw_level()
{
    for (int i = 0; i < room_count; i++) {
        DrawRectangle(rooms[i].x * WINDOW_WIDTH / col_count, rooms[i].y * WINDOW_HEIGHT / col_count,
                        WINDOW_WIDTH / col_count, WINDOW_HEIGHT / row_count, rooms[i].color);
        DrawText(std::to_string(rooms[i].distance_from_start).c_str(),
                    rooms[i].x * WINDOW_WIDTH / col_count + 10,
                    rooms[i].y * WINDOW_HEIGHT / col_count + 10, 32, WHITE);
    }

    for (int i = 0; i < col_count; i++) {
        for (int j = 0; j < row_count; j++) {
            DrawRectangleLines(i * WINDOW_WIDTH / col_count, j * WINDOW_HEIGHT / row_count,
                                WINDOW_WIDTH / col_count, WINDOW_HEIGHT / row_count, LIGHTGRAY);
        }
    }
}